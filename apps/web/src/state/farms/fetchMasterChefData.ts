...
const masterChefFarmCalls = async (farm: SerializedFarm) => {
  ...
  // TODOXXX
  const multiCallChainId = farmFetcher.isTestnet(quoteToken.chainId) ? ChainId.OKCTEST : ChainId.BSC
...
export const fetchMasterChefData = async (farms: SerializedFarmConfig[], chainId: number): Promise<any[]> => {
  ...
  // TODOXXX
  const multiCallChainId = farmFetcher.isTestnet(chainId) ? ChainId.OKCTEST : ChainId.BSC
