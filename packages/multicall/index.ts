...
export const multicallAddresses = {
  ...
  // 添加multicall合约
  65: '0x4Ca509E38ded6a07b4c55e6b0afB0777fC0095FF',
}
...
export function createMulticall<TProvider>(provider: ({ chainId }: { chainId?: number | undefined }) => TProvider) {
  const multicall: MultiCall = async (abi: any[], calls: Call[], chainId = ChainId.OKCTEST) => {
  ...
  // TODOXXX 替换ChainId.OKCTEST
  const multicallv2: MultiCallV2 = async ({ abi, calls, chainId = ChainId.OKCTEST, options, provider: _provider }) => {
  ...
  // TODOXXX 替换ChainId.OKCTEST
  const multicallv3 = async ({ calls, chainId = ChainId.OKCTEST, allowFailure, overrides }: MulticallV3Params) => {
    const multi = getMulticallContract(chainId, provider({ chainId }))
    if (!multi) throw new Error(`Multicall Provider missing for ${chainId}`)
